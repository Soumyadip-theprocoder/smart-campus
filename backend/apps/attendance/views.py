...
class AdminCSVExportView(APIView):
    ...
    def get(self, request):
        if not request.user.is_admin:
            return Response({'error': 'Only admins can export full CSV.'}, status=status.HTTP_403_FORBIDDEN)
            
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="attendance_report.csv"'
        
        writer = csv.writer(response)
        writer.writerow(['Date', 'Student ID', 'Student Name', 'Subject Code', 'Status', 'Method'])
        
        attendances = Attendance.objects.select_related('student__user', 'subject').all().order_by('-date')
...
class StudentPDFExportView(APIView):
    ...
    def get(self, request, student_id):
        ...
        p.drawString(50, height - 50, f"Attendance Report: {student.user.get_full_name()}")
        p.setFont("Helvetica", 12)
        p.drawString(50, height - 70, f"Enrollment Number: {student.enrollment_number}")
        p.drawString(50, height - 90, f"Date Generated: {date.today().strftime('%Y-%m-%d')}")
        ...
        for subject in subjects:
            ...
            p.drawString(50, y_position, f"{subject.code} - {subject.name}: {attended}/{total} classes ({pct}%)")
            y_position -= 20
        ...
        response['Content-Disposition'] = f'attachment; filename="attendance_{student.enrollment_number}.pdf"'
        response.write(pdf)
        return response
