...
class AttendanceModelTests(TestCase):
    ...
    def setUp(self):
        self.faculty_user = User.objects.create_user(
            email='fac@test.com', username='fac', password='pass', role='faculty',
        )
        self.faculty = Faculty.objects.create(
            user=self.faculty_user, employee_id='FAC_T1',
            department='CS', designation='Professor',
        )
        self.student_user = User.objects.create_user(
            email='stu@test.com', username='stu', password='pass', role='student',
        )
        self.student = Student.objects.create(
            user=self.student_user, enrollment_number='STU_T1',
            department='CS', semester=3,
        )
        self.subject = Subject.objects.create(
            code='TEST101', name='Test Subject',
            faculty=self.faculty, credits=3,
            required_capacity=30, sessions_per_week=3,
        )
    ...
    def test_unique_constraint(self):
        ...
        Attendance.objects.create(
            ...
            date=date.today(), status=Attendance.Status.PRESENT,
        )
        with self.assertRaises(Exception):
            Attendance.objects.create(
                student=self.student, subject=self.subject,
                date=date.today(), status=Attendance.Status.ABSENT,
            )
...
class AttendanceAPITests(TestCase):
    ...
    def setUp(self):
        ...
        self.faculty = Faculty.objects.create(
            user=self.fac_user, employee_id='FAC_T2',
            department='CS', designation='Professor',
        )
        ...
        self.student = Student.objects.create(
            user=self.stu_user, enrollment_number='STU_T2',
            department='CS', semester=3,
        )
        self.subject = Subject.objects.create(
            code='ATT101', name='Attendance Test Subject',
            faculty=self.faculty, credits=3,
            required_capacity=30, sessions_per_week=3,
        )
    ...
    def test_student_sees_only_own_attendance(self):
        ...
        other_student = Student.objects.create(
            user=other_user, enrollment_number='STU_OTHER',
            department='CS', semester=3,
        )
        Attendance.objects.create(
            student=self.student, subject=self.subject,
        ...
        for record in results:
            self.assertEqual(record['student_name'], 'Test User' if 'student_name' in record else True, True)
    ...
    def test_mark_attendance(self):
        ...
        self.assertTrue(Attendance.objects.filter(
            student=self.student, subject=self.subject, date=date.today(),
        ).exists())
    ...
    def test_attendance_report(self):
        ...
        response = self.client.get(
            reverse('attendance:report', args=[self.student.id])
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class LowAttendanceServiceTests(TestCase):
    ...
    def setUp(self):
        ...
        self.faculty = Faculty.objects.create(
            user=self.fac_user, employee_id='FAC_S1',
            department='CS', designation='Professor',
        )
        self.subject = Subject.objects.create(
            code='SVC101', name='Service Test', faculty=self.faculty,
            credits=3, required_capacity=30, sessions_per_week=3,
        )

    def _create_student_with_attendance(self, email, enrollment, present, absent):
        user = User.objects.create_user(
            email=email, username=enrollment, password='pass', role='student',
        )
        student = Student.objects.create(
            user=user, enrollment_number=enrollment, department='CS', semester=3,
        )
        for i in range(present):
            Attendance.objects.create(
                student=student, subject=self.subject,
                date=date.today() - timedelta(days=i),
                status='present',
            )
...
class RobustnessAttendanceTests(TestCase):
    ...
    def setUp(self):
        ...
        self.faculty = Faculty.objects.create(
            user=self.fac_user, employee_id='FAC_R1',
            department='CS', designation='Professor',
        )
        self.stu_user = User.objects.create_user(
            email='stu@test.com', username='stu', password='pass', role='student',
        ...
        self.student = Student.objects.create(
            user=self.stu_user, enrollment_number='STU_R1',
            department='CS', semester=3,
        )
        self.subject = Subject.objects.create(
            code='ROB101', name='Robustness Test Subject',
            faculty=self.faculty, credits=3,
            required_capacity=30, sessions_per_week=3,
        )
