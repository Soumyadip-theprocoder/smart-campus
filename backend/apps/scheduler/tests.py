...
class CSPSolverBasicTests(TestCase):
    ...
    def _make_rooms(self, data):
        return [
            {'id': i + 1, 'room_number': d['number'], 'capacity': d['capacity'],
             'room_type': d.get('type', 'lecture'), 'building': d.get('building', 'Main')}
            for i, d in enumerate(data)
        ]
    ...
    def test_no_faculty_conflict(self):
        ...
        slots = self._make_slots(['MON', 'TUE', 'WED', 'THU', 'FRI'], [(9, 10), (10, 11)])
        ...
        for entry in timetable:
            ...
            key = (fid, ts)
            self.assertNotIn(key, time_slots_used,
                             f"Faculty {fid} double-booked at {ts}")
    ...
    def test_no_room_conflict(self):
        ...
        for entry in timetable:
            ...
            self.assertNotIn(key, room_slots,
                             f"Room {entry['room_number']} double-booked")
            room_slots.add(key)
    ...
    def test_same_subject_not_twice_per_day(self):
        ...
        self.assertEqual(len(days_used), len(set(days_used)),
    ...
    def test_impossible_schedule_returns_empty(self):
        ...
        self.assertEqual(len(timetable), 0, "Should return empty for impossible schedule")
    ...
    def test_locked_entries_respected(self):
        ...
        locked = [{'subject_id': 1, 'room_id': 2, 'time_slot_id': 1}]  # CS1 locked to R2 on MON 9-10

        csp = ScheduleCSP(subjects, rooms, slots, locked_entries=locked)
        timetable = csp.get_timetable()

        self.assertEqual(len(timetable), 2)
        locked_entry = [e for e in timetable if e['day'] == 'MON' and e['start_time'] == '09:00']
...
class CSPSolverEdgeCaseTests(TestCase):
    ...
    def test_single_subject_single_session(self):
        subjects = [{'id': 1, 'code': 'SOLO', 'name': 'Solo', 'faculty_id': 1,
                      'required_capacity': 10, 'sessions_per_week': 1}]
    ...
    def test_zero_rooms(self):
        subjects = [{'id': 1, 'code': 'SOLO', 'faculty_id': 1, 'sessions_per_week': 1, 'required_capacity': 10}]
        slots = [{'id': 1, 'day': 'MON', 'start_time': '09:00', 'end_time': '10:00'}]
    ...
    def test_conflicting_locked_entries(self):
        subjects = [{'id': 1, 'code': 'S1', 'faculty_id': 1, 'sessions_per_week': 1, 'required_capacity': 10}]
        rooms = [{'id': 1, 'room_number': 'R1', 'capacity': 20}]
        slots = [{'id': 1, 'day': 'MON', 'start_time': '09:00', 'end_time': '10:00'}]
        ...
        locked = [
            ...
            {'subject_id': 2, 'room_id': 1, 'time_slot_id': 1}, # Subject 2 doesn't exist but let's simulate a bad lock
        ]
